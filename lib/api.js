...
export const  getAllTeachersByLanguage = async (languageId) => {
  const data = await client.fetch(
...
export async function getAllRegistrationsWithSlug() {
  const data = await client.fetch(`*[_type == "registration"]{ 'slug': _id }`)
  return data
}
...
export async function getAllLanguageData() {
  const data = await client.fetch(
    `*[_type == "language"] {
      _id,
      title,
